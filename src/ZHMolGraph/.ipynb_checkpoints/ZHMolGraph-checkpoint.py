...
class ZHMolGraph():
    ...
    def __init__(self,
        ...
        self.reqId=reqId
        print("reqId in ZHMolGraph",self.reqId)
    ...
    def read_input_files(self, input_location):
        ...
        if input_location.split('.')[-1] == 'pkl':
            ...
            raise ValueError("Unknown input file type, only pkl and csv are supported")
    ...
    def get_test_graphsage_embeddings(self, train_set, test_set, rnas, proteins, dataset, rnas_length, proteins_length, embedding_type, graphsage_embedding=1, graphsage_model_path=None):
        ...
        #     total_feat.to_csv(output_feats_file, sep='\t', index=False, header=False)
            ...
            self.graphsage_embeddings = self.run_graphsage_model(dataSet=dataset, seed=64, cuda=True,
                                                                      config='src/graphsage_src/experiments.conf',
    ...
    def get_unseen_graphsage_embeddings(self, config='src/graphsage_src/experiments.conf',
        ...
        ### 输入模型
        # model_path = os.path.join("src",model_path)
        # print(f"Loading GraphSAGE model from {model_path}")
        ...
        # print(f"feat_data'\n'{feat_data.shape}")
        # print(self.reqId)
        os.makedirs('src/data/input/' +self.reqId+'_'+ unseen_dataset +'_graphsage_dataset/', exist_ok=True)
        output_feats_file = os.path.join("src","data","input",self.reqId+'_'+unseen_dataset + '_graphsage_dataset/' + unseen_dataset + '_feats' + '.txt')
        # print("output_feats_file", output_feats_file)
        test_rnas = rnas
        ...
        if not os.path.exists(output_feats_file):
            ...
            unseen_dataset_path = 'src/interactions/' + 'dataset_RPI_' + unseen_dataset + '_RP.csv'
            ...
            # misc=unseen_dataset+'_graphsage_dataset/'+ unseen_dataset + '_graphsage_dataset/'
            # print("misc",misc)
            ...
            total_interaction_df.to_csv('src/data/input/'+self.reqId+'_'+unseen_dataset + '_graphsage_dataset/' + unseen_dataset + '_total_interactions_seq_list' + '.txt',
                                        sep='\t', index=False, header=False)
        ...
        ds = model_dataset
        dataCenter = DataCenter(config,self.reqId)
    ...
    def run_graphsage_model(self, dataSet=None, seed=64, cuda=False, config='src/graphsage_src/experiments.conf', embedding_type=None, graphsage_model_path=None):
    ...
    def run_graphsage_experiment(self, dataSet=None, agg_func='MEAN', epochs=3, b_sz=20, seed=64, cuda=False,
                                 ...
                                 config='src/graphsage_src/experiments.conf',embedding_type=None):
    ...
    def get_rnafm_embeddings(self, prediction_interactions = None, embedding_dimension = 640,
        ...
        # Load RNA-FM model
        # model,alphabet = fm.pretrained.rna_fm_t12('/raid/apitempfiles/siarna/RNA-FM_pretrained.pth')
        model, alphabet = fm.pretrained.rna_fm_t12('/usr/src/app/src/data/input/RNA-FM_pretrained.pth')
    ...
    def get_ProtTrans_embeddings(self, prediction_interactions=None, embedding_dimension=1024, replace_dataframe=True,
        ...
        # Load the model
        model = T5EncoderModel.from_pretrained(LOCAL_MODEL_DIR).to(device)
