...
def main(request_Id: str):
    current_dir="src/data/input/"
    print("Preprocessing-----------")
    ...
    # os.chdir('src')
    # Read mrna file
    mrna_file = f"src/data/input/{reqId}_mRNA.fa"
    mrna_map = []
    ...
    # Read sirna file
    sirna_file = f"src/data/input/{reqId}_siRNA.fa"  # assuming FASTA format for siRNAs
    sirna_map = []
    ...
    output_path_dir = os.path.join(current_path, f"src/data/input/{reqId}_preprocess")
    ...
    for ind, row in mrna_df.iterrows():
    	...
    	proc = subprocess.Popen(['RNAfold','-p',"--id-prefix=" + id_name], 
    		stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text = True,cwd="src/data/input")
    
    	output, error = proc.communicate(seq)
    	
    path_mrna = "src/data/input/"
    ...
    for file in files_mrna:
        ...
        start_flag = False
        os.makedirs(f"src/data/input/{reqId}_RNAfold_bp_file_mrna", exist_ok=True)
        f_mrna = open(f"src/data/input/{reqId}_RNAfold_bp_file_mrna/" + file + ".bpp", "w")
    ...
    file_paths_mrna = glob.glob(f'src/data/input/{reqId}_RNAfold_bp_file_mrna/*dp.ps.bpp')
    ...
    for file_path in file_paths_mrna:
        ...
        directory_path_mrna = f'src/data/input/{reqId}_RNAfold_reduced_matrix_mrna'+ str(n_components)
    
        if not os.path.exists(directory_path_mrna):
            os.mkdir(directory_path_mrna)
    
        np.save(f'src/data/input/{reqId}_RNAfold_reduced_matrix_mrna'+ str(n_components) + '/' + file_path.split('/')[-1].split('_dp.ps.bpp')[0]+'.npy',reduced_data)
            
    #------------------4---------------------
    
    path_mrna = f"src/data/input/{reqId}_RNAfold_reduced_matrix_mrna100"
    ...
    for file in files_sirna:
        ...
        start_flag = False
        os.makedirs(f"src/data/input/{reqId}_RNAfold_bp_file_sirna", exist_ok=True)
        f_sirna = open(f"src/data/input/{reqId}_RNAfold_bp_file_sirna/" + file + ".bpp", "w")
        for line in temp:
            line = line.strip()
            if "start of base pair probability data" in line:
                start_flag = True
    ...
    file_paths_sirna = glob.glob(f'src/data/input/{reqId}_RNAfold_bp_file_sirna/*dp.ps.bpp')
    ...
    for file_path in file_paths_sirna:
        ...
        directory_path_sirna = f'src/data/input/{reqId}_RNAfold_reduced_matrix_sirna'+ str(n_components_sirna)
    
        if not os.path.exists(directory_path_sirna):
            os.mkdir(directory_path_sirna)
    
        np.save(f'src/data/input/{reqId}_RNAfold_reduced_matrix_sirna'+ str(n_components_sirna) + '/' + file_path.split('/')[-1].split('_dp.ps.bpp')[0]+'.npy',reduced_data)
                
    #------------------4--------------
    path_sirna = f"src/data/input/{reqId}_RNAfold_reduced_matrix_sirna6"
    files_sirna = os.listdir(path_sirna)
    ...
    sirna_map = {
        ...
        for rec in SeqIO.parse(f"src/data/input/{reqId}_siRNA.fa", "fasta")
    }
    
    # --- Load mRNA sequences ---
    mrna_map = {
        rec.id: str(rec.seq).upper().replace("T", "U")
        for rec in SeqIO.parse(f"src/data/input/{reqId}_mRNA.fa", "fasta")
    }
    ...
    print("Creating file")
    eff_df[final_cols].to_csv(f"src/data/input/{reqId}_inference_siRNA_mRNA_pairs.csv", index=False)
